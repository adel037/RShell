...
OutputReClear::OutputReClear(Base* userLeft, Base* userRight) {
        this->left = userLeft;
        this->right = userRight;
        this->parent = nullptr;
        this->classType = "OutputReClear";
}

void OutputReClear::execute() {
...
void OutputReClear::initializeFileDesc(int &savestdout, int &file_desc){
        savestdout = dup(1);
        file_desc  = open(this->right->getFileName(), O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
        dup2(file_desc, 1);
}

void OutputReClear::restore_CloseFile(int &savestdout, int &file_desc){
        dup2(savestdout, 1);
        close(file_desc);
}
